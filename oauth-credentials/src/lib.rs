...
impl<T> Credentials<T> {
    /// Maps an `Credentials<T>` to `Credentials<U>` by applying a function to contained values.
    ///
    /// # Example
    ///
    /// ```edition2018
    ...
    /// # */
    /// # async fn f() {
    /// # async fn get_temporary_credentials() -> Credentials { Credentials::new("".into(), "".into()) }
