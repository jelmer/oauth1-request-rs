...
use signature_method::plaintext::PlaintextSign;
use signature_method::{Sign, SignatureMethod};
use util::*;

/// A type that creates a signed `Request`.
#[derive(Debug)]
pub struct Signer<S, State = NotReady> {
    inner: Inner<S>,
...
pub type PlaintextSigner<State = NotReady> = Signer<PlaintextSign, State>;

cfg_if! {
    if #[cfg(feature = "hmac-sha1")] {
        pub use signature_method::HmacSha1;
        use signature_method::hmac_sha1::HmacSha1Sign;

        /// A version of `Signer` that uses the `HMAC-SHA1` signature method.
        pub type HmacSha1Signer<State = NotReady> = Signer<HmacSha1Sign, State>;
    }
}
...
impl<S: Sign> Signer<S, NotReady> {
    ...
    pub fn with_signature_method<'a, SM>(
        signature_method: SM,
        method: &str,
        uri: impl Display,
        consumer_secret: &str,
    ...
    where
        SM: SignatureMethod<Sign = S>,
        S: Sign<SignatureMethod = SM>,
    {
        Self::new_(
            method,
            uri,
    ...
    pub fn form_with_signature_method<'a, SM>(
        signature_method: SM,
        method: &str,
        uri: impl Display,
        consumer_secret: &str,
    ...
    fn new_(
        ...
        cs: &str,
        ts: Option<&str>,
        sm: S::SignatureMethod,
        q: bool,
    ) -> Self {
        let mut sign = S::new(percent_encode(cs), ts.map(percent_encode), sm);
    ...
    fn append_oauth_params_(mut self, ck: &str, opts: &Options) -> Signer<S, Ready> {
        macro_rules! append {
...
impl<S: Sign, State> Signer<S, State> {
...
impl Request {
    ...
    fn new_<SM: SignatureMethod>(
        ...
        let mut signer = Signer::<SM::Sign>::new_(method, uri, cs, ts, sm, q);
...
mod tests {
    ...
    impl<S: Sign> Sign for InspectSign<S> {
        type SignatureMethod = Inspect<S::SignatureMethod>;
        type Signature = S::Signature;

        fn new(cs: impl Display, ts: Option<impl Display>, sm: Self::SignatureMethod) -> Self {
            println!("cs: {:?}", cs.to_string());
            println!("ts: {:?}", ts.as_ref().map(ToString::to_string));
            InspectSign(S::new(cs, ts, sm.0))
        }
